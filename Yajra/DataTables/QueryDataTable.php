...
use Yajra\DataTables\Utilities\Config as DtConfig;
...
class QueryDataTable extends DataTableAbstract
{
    ...
    {
        $this->query      = $builder;
        $this->request    =new Request(); //app('datatables.request');
        $this->config     =new DtConfig();// app('datatables.uilities.config');
    ...
    public function make($mDataSupport = true)
    {
        try {
          ...
          // return $exception ;$this->errorResponse($exception);
    ...
    {
        return Str::contains(Str::lower($builder->toSql()), ['union', 'having', 'distinct', 'order by', 'group by',"where","where in",'join']);
    }
