    ...
    protected function getSorter(array $criteria)
    {
        $sorter = function ($a, $b) use ($criteria) {
            foreach ($criteria as $orderable) {
                ...
                if (is_numeric($first[$column] ? $first[$column]: null) && is_numeric($second[$column] ? $second[$column]: null)) {
                    ...
                    $cmp = strnatcasecmp($first[$column] ? $first[$column]: null, $second[$column] ?$second[$column]: null);
                } else {
                    $cmp = strnatcmp($first[$column] ?$first[$column]: null, $second[$column] ?$second[$column]: null);
                }
                if ($cmp != 0) {
                    return $cmp;
                }
