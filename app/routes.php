				...
				if($arg3!=""){
					...
					if(in_array($arg2, $vars)){
						...
						if(in_array($arg3, $sufraticities )){
							Route::get('home-delivery/'.$arg3,function() use ($arg3,$langstring){ return Redirect::to($langstring.$arg3.'/home-delivery',301); });
							Route::get('finedining/'.$arg3,function() use ($arg3,$langstring){ return Redirect::to($langstring.$arg3.'/fine-dining',301); });
							Route::get('offers/'.$arg3,function() use ($arg3,$langstring){ return Redirect::to($langstring.$arg3.'/offers',301); });
							Route::get('hotels/'.$arg3,function() use ($arg3,$langstring){ return Redirect::to($langstring.$arg3.'/hotels',301); });
							Route::get('catering/'.$arg3,function() use ($arg3,$langstring){ return Redirect::to($langstring.$arg2.'/catering',301); });
							Route::get('sufrati-favorites/'.$arg3,function() use ($arg3,$langstring){ return Redirect::to($langstring.$arg3.'/recommended',301); });
							Route::get('cuisines/'.$arg3,function() use ($arg3,$langstring){ return Redirect::to($langstring.$arg3.'/cuisines',301); });
							Route::get('shisha/'.$arg3,function() use ($arg3,$langstring){ return Redirect::to($langstring.$arg3.'/sheesha',301); });
							Route::get('gallery/'.$arg3,function() use ($arg3,$langstring){ return Redirect::to($langstring.$arg3.'/gallery',301); });
							Route::get('food/'.$arg3,function() use ($arg3,$langstring){ return Redirect::to($langstring.$arg3,301); });
						}
					}
				}
			}else{
				Route::get('blog/{any?}',function() use ($langstring){ return Redirect::to($langstring.'blog',301); });
				Route::get('recipe/{any?}',function() use ($langstring){ return Redirect::to($langstring.'recipes',301); });
				Route::get('contact',function() use ($langstring){ return Redirect::to($langstring.'contact-us',301); });
				Route::get('search/{any?}',function() use ($langstring){ return Redirect::to($langstring.'',301); });
				Route::get('user/{id}',function($id) use ($langstring){  return Redirect::to($langstring.'user/'.$id,301);});
				...
				if(in_array($arg2, $sufraticities )){
					...
					if($arg3==""){
						...
						if(in_array($arg3, $sfcuisine)){
							Route::get($arg2.'/'.$arg3,function() use($arg2,$arg3,$langstring){ return Redirect::to($langstring.$arg2.'/'.$arg3.'/restaurants');});
						}else{
							...
							if(count($branch)>0){
								...
								if(count($rest)>0){
									Route::get($arg2.'/'.$arg3,function() use($arg2,$rest,$branch,$langstring){ return Redirect::to($langstring.$arg2.'/'.$rest->seo_url.'/'.$branch->url);});	
								}
							}
						}
					}
...
Route::group(array('prefix' => $locale), function()  {
	...
	$normalpages=array('privacy-terms','login','videos','video','user','add','settings','userhelp','updatedb','blog','recipes','article','locations','forgot','reset','resetpassword','logout','step','reactivate','welcome','preference','ajax','press','clearnotifications','clearnotif','ads','contact-us','page','poll','usersuggestions','likesuggestions','userpreference','aj','invite','catering-terms','style','updatemenuimage','sitemap','test');
