                        ...
                        foreach ($lists as $list) {
                            ...
                            <tr>
                                ...
                                <td class="sufrati-action">
                                    <a class="btn btn-xs btn-info mytooltip m-1" href="{{ route('adminrestaurants/videoform/',$list->id).'?rest_ID='.$rest->rest_ID; }}" title="Edit Content">
                                        <i data-feather="edit"></i> 
                                    </a>
                                    <?php
                                    if ($list->status == 0) {
                                        ?>
                                        <a class="btn btn-xs btn-danger mytooltip m-1" href="{{ route('adminrestaurants/videostatus/',$list->id).'?rest_ID='.$rest->rest_ID; }}" title="Activate "><i data-feather="minus-circle"></i></a>
                                        <?php
                                    } else {
                                        ?>
                                        <a class="btn btn-xs btn-info mytooltip m-1" href="{{ route('adminrestaurants/videostatus/',$list->id).'?rest_ID='.$rest->rest_ID; }}" title="Deactivate"><i data-feather="plus-circle"></i></a>
                                        <?php
                                    }
                                    ?>
                                    <a  class="btn btn-xs btn-danger mytooltip m-1 cofirm-delete-button" href="#" link="{{ route('adminrestaurants/videodelete/',$list->id).'?rest_ID='.$rest->rest_ID; }}" title="Delete">
