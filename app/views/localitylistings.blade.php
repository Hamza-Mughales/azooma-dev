<!doctype html>
<html lang="<?php echo $lang;?>">
<head>
    @include('inc.metaheader',$meta)
    <?php
    if(isset($metastring)){
        echo $metastring;
    }
    ?>
</head>
<body itemscope itemtype="http://schema.org/WebPage" class="<?php if($lang == 'ar'){ echo 'rtl'; } ?>">
    @include('inc.header')
    <?php
    ($lang=="en")?$cityname=stripcslashes($city->city_Name):$cityname=stripcslashes($city->city_Name_ar);
    ?>

        {{-- Breadcrumb Section Start --}}
        <section class="Breadcrumb">
            <div class="container">
                <div class="row">
                    <div class="col-md-6 col-xs-12">
                        <ul class="breadcrumb-nav">
                            <li>
                                <i class="fa fa-home"></i> <a href="<?php echo Azooma::URL('');?>" title="<?php echo Lang::get('messages.azooma');?>">
                                <?php echo Lang::get('messages.azooma'); ?></a>
                            </li>
                            <li>
                                <a href="<?php echo Azooma::URL($city->seo_url);?>" title="<?php echo Lang::choice('messages.restaurants',2).' '.Lang::get('messages.inplace',array('name'=>$cityname));?>">
                                    <?php echo ($lang=="en")?$cityname=stripcslashes($city->city_Name):$cityname=stripcslashes($city->city_Name_ar); ?>
                                </a>
                            </li>
                            <li class="active">
                                <?php echo Lang::get('messages.browse_by_locations');?>
                            </li>
                        </ul>
                    </div>
                    <div class="col-md-6 col-xs-12">
                        <div class="breadcrumb-social">
                            <div class="social">
                                <a href="https://twitter.com/share"><i class="fa fa-twitter"></i> Tweet</a>
                            </div>
                            <div class="social">
                                <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo Request::url();?>"><i
                                        class="fa fa-facebook"></i> Share</a>
                                {{-- <div class="fb-share-button" data-layout="button_count" data-href="<?php echo Request::url();?>"></div> --}}
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    {{-- SiteMap Section Start --}}
    <section class="site-map">
        <div class="container">
            <div class="row">

                <div class="col-md-12">
                    <div class="country-block">
                        <h1 class="inline-block section-title mb-4">
                            <?php echo Lang::get('messages.localities').' '.Lang::get('messages.inplace',array('name'=>$cityname));?>
                        </h1>
                        <ul class="city-list">
                                <?php
                                $i=0;
                                if(count($localities)>0){
                                    $i=0;
                                    foreach ($localities as $locality) {
                                        if($i%21==0){
                                            ?>
                                       
                                            <li>
                                                <a href="<?php echo Azooma::URL($city->seo_url.'/'.$locality->seo_url.'/restaurants');?>" title="<?php echo ($lang=="en")?stripcslashes($locality->district_Name):stripcslashes($locality->district_Name_ar); echo ' '.Lang::choice('messages.restaurants',2).' '.Lang::get('messages.inplace',array('name'=>$cityname)); ?>">
                                                   
                                                    <h2> <?php 
                                                        echo ($lang=="en")?stripcslashes($locality->district_Name):stripcslashes($locality->district_Name_ar);
                                                        ?> - <span><?php echo $locality->total;?></span> </h2>
                                                </a> 
                                            </li>
                                            <?php } } }?>
                                        

                                </ul>
                    </div>
                </div>
            </div></div></section>


    @include('inc.footer')
    <script type="text/javascript">
        require(['popular'],function(){});
    </script>
</body>
</html>