...
class User extends Eloquent
{
	...
	public static function register($name, $email, $password, $phone = 0, $cityid, $countryid, $google, $facebook = 0, $countrycode = '')
	{
	...
	public static function getActivities($userid = 0, $limit = 20, $offset = 0)
	{
	...
	{
		return DB::table('rating_info')->where('user_ID', $userid)->where('rest_ID', $restid)->where(function ($query) use ($cityid) {
	...
	{
		$checkrated = DB::table('rating_info')->where('user_ID', $userid)->where('rest_ID', $restid)->where(function ($query) use ($cityid) {
			$query->where('city_ID', '=', $cityid)->orWhere('city_ID', '=', 0);
		})->count();
		$data = array(
			'user_ID' => $userid,
	...
	public static function LikeRest($rest = 0, $user = 0, $city = 0, $status = 0)
	{
		$data = array(
			'rest_ID' => $rest,
			'user_ID' => $user,
	...
	public static function likePhoto($photo = 0, $user = 0)
	{
		$ip = Azooma::getRealIpAddr();
		$data = array(
			'image_ID' => $photo,
