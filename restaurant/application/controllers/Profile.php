...
class Profile extends MY_Controller {
    ...
    public function index(){
        ...
        $data['css']='admin,docs,chosen';
        $data['js']='chosen,validate';
        $data['main']='rest_details';
        $data['side_menu'] = array("profile");
            ...
            if($this->input->post('rest_ID')){
                if(is_uploaded_file($_FILES['rest_Logo']['tmp_name'])){
					...
					if($logo != 'default_logo.gif'){
                        ...
                        //check file size
	                    list($width, $height, $type, $attr) = getimagesize('../uploads/logos/' . $logo);
						...
						$this->images->squareThumb('../uploads/logos/' . $logo,'../uploads/logos/70/' . $logo,70);
                    }					
                }else{
                ...
                $this->session->set_flashdata('message',$_POST['rest_Name'].' Updated successfully');                
                ...
                returnMsg("success","profile",lang("saved_sueccessfuly"));
            }else{
               
                returnMsg("error","profile",lang("saved_error"));
