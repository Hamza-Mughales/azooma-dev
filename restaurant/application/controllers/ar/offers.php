...
class Offers extends CI_Controller {
    ...
    function categories($item=0){
        ...
        $config['base_url'] = base_url().'ar/offers/categories?limit='.$limit;
        ...
        $data['css']='ar';
        $data['pagetitle']=(htmlspecialchars($restdata['rest_Name_Ar'])).' - انواع العروض';
        $data['main']='ar/offerscategories';
        $this->load->view('ar/template',$data);
    }
    ...
    function form($id=0){
        ...
        if($id!=0){
            ...
            $data['title']=$data['pagetitle']='إضافة عرض جديد';
        }
            ...
            if(in_array(10,$sub_detail_permissions) || in_array(11,$sub_detail_permissions)){
               $totalOffers=$this->MRestBranch->getAllTotalOffers($rest);
                ...
                if( $totalOffers >= $availableOffers ){
                    if($availableOffers==3){
                        $this->session->set_flashdata('error', 'عفوا! اشتراكك الحالى لا يسمح لك إضافة اكثر من ثلاثة عروض فقط. للحصول على مزيد من الاستفسار اضغط هنا');
                    }else{
                        $this->session->set_flashdata('error', 'عفوا! اشتراكك الحالى لا يسمح لك إضافة اكثر من عرض واحد فقط. للحصول على مزيد من الاستفسار اضغط هنا');
                    }
            ...
            if($this->input->post('id')){
                if(is_uploaded_file($_FILES['image']['tmp_name'])){
                    ...
                    if($width<200 || $height<200){
                        ...
                        redirect("ar/offers/form?rest=".$rest);
                    }
    ...
    function status($id=0){
        ...
        if($offer['status']==1){    
            $this->session->set_flashdata('message','العرض الغير نشط بنجاح');            
        }else{
            $this->session->set_flashdata('message','العرض المنشط بنجاح ');
        }
        redirect('ar/offers');
    }
