		...
		for ($x=0; $x < $totaladdr; $x++) {
			...
			if (isset($msgdata->varsearch)) {
				...
				for ($y=0; $y < count($msgdata->varsearch); $y++) {
					...
					if($msgdata->varsearch[$y]=="[hashsubscriberid]"){
						$thisreplacement = trim(base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256,'asdfghungry', $thisreplacementarr[$x], MCRYPT_MODE_ECB, mcrypt_create_iv(mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_ECB), MCRYPT_RAND))));
					}else{
